...
public class NotificationUtils {
    ...
    public static NotificationCompat.Action ignoreReminderAction(Context context) {
        Intent intentWaterReminderIntentService = new Intent(context, WaterReminderIntentService.class);
        intentWaterReminderIntentService.setAction(ReminderTasks.ACTION_DISMISS_NOTIFICATION);

        PendingIntent ignoreReminderPendingIntent = PendingIntent.getService(context,
                ACTION_IGNORE_PENDING_INTENT_ID ,
                intentWaterReminderIntentService, PendingIntent.FLAG_UPDATE_CURRENT);
    ...
    public static NotificationCompat.Action drinkWaterAction(Context context) {
        Intent intentWaterReminderIntentService = new Intent(context, WaterReminderIntentService.class);
        intentWaterReminderIntentService.setAction(ReminderTasks.ACTION_INCREMENT_WATER_COUNT);

        PendingIntent pendingIntent = PendingIntent.getService(context,
                ACTION_DRINK_PENDING_INTENT_ID,
                intentWaterReminderIntentService, PendingIntent.FLAG_UPDATE_CURRENT);

        NotificationCompat.Action incrementNotification = new NotificationCompat.Action(R.drawable.ic_local_drink_black_24px,
                "I did it!",
                pendingIntent);

        return incrementNotification;
    }
